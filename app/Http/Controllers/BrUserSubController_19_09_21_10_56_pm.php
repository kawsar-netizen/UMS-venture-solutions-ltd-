...
class BrUserSubController extends Controller
{
        ...
        if(count($req_check_count) > 0){
          ...
          if($req_check->id!=NULL and !empty($req_check->id)){
            ...
            // return response()->json($response);
             ...
             echo "0<>Please select Parameter before submit";
             die();
 ...
 if($this->checkRtgsParam($request->input('form_serialize_data')) === false and $requestTypeValueArra['6']['request_type_id']==33){
            // $response_request_type = [
            //     "type"       => "warning",
            //     "status"     => 400,
            //     "success"    => false,
...
public function checkRtgsParam($form_serialize_data){
  $data_array = explode("&", $form_serialize_data);
