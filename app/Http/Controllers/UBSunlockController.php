...
if(count($user_get_data)>0){
  ...
  public function ubsUnlock_Store(Request $request)
  {
    ...
    $division_id= Auth::user()->division_name;
  ...
  public function authorizeList()
  {
    ...
    elseif($usr_branch=='202' and $usr_primary_id=='20289')
    {
      $authorize_get_data = DB::table('ubs_unlock_request')->where('status',0)->where('maker_user_id','!=',$usr_primary_id)->get();
      return view('ubs_unlock.authorize_list', compact('authorize_get_data'));
    }
    else
    {
      $authorize_get_data = DB::table('ubs_unlock_request')->where('status',0)->where('maker_user_id','!=',$usr_primary_id)->where('br_code','=',$usr_branch)->where('division_id' ,'=', $division_id)->get();
      return view('ubs_unlock.authorize_list', compact('authorize_get_data'));
    }
  ...
  public function changeStatus_authorize($id)
  {
    ...
    if($responseData['is_success'] == 'success') {
        DB::update(DB::RAW("UPDATE ubs_unlock_request SET status = 1,autho_date='".$currentTimestamp."',checker_user_id='".$authUserId."' WHERE id = " . $id));
  ...
  public function DoLogOutUser($userId){
      ...
      $inquiry_url="https://wsapi.dhakabank.com.bd/DblUserServices/FubsUserServices.asmx?WSDL";
          $curl = curl_init();
  ...
  public function changeStatus_decline($id)
  {
    ...
    DB::update(DB::RAW("UPDATE ubs_unlock_request SET status = 2,autho_date='".$currentTimestamp."',checker_user_id='".$authUserId."' WHERE id = " . $id));
