...
trait AuthenticatesUsers
{
    ...
    private function adLogin($user_id, $password){  
        ...
        if($user_count > 0){
            $user_info = User::select('login_level')->where('user_id', $user_id)->first();
    ...
    public function checkUserLoginTypeNonAD($user_id){
        $user_count = User::where('user_id', $user_id)->count();
        if($user_count > 0 ){
    ...
    {   
        if($this->checkUserLoginTypeNonAD($request->input('user_id')) === true){         
            Input::merge(['password' => $request->input('ldap_password')]);
        }
        file_put_contents("nishad.txt", "nishad");
